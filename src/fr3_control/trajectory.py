...
class ReferenceTrajectory:
    ...
    Attributes:
        ...
        ee_rotation: 全程保持不变的参考末端旋转矩阵，形状为 ``(3, 3)``。
    """
    ...
    def sample(self, index: int) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        """返回指定控制周期的关节参考量和末端目标位置。

        Args:
            index: 控制周期在轨迹数组中的下标。

        Returns:
            依次返回参考关节角、角速度、角加速度和末端位置。
        """
        return self.q[index], self.qd[index], self.qdd[index], self.ee_position[index]
...
def task_position(t: float, home_position: np.ndarray) -> np.ndarray:
    ...
    center = home_position + np.array([0.0, 0.0, -0.05])
